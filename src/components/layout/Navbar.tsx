...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav aria-label="Main Navigation" className="sticky top-0 z-50 w-full bg-ivory/90 backdrop-blur-md border-b border-gold/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="flex md:hidden items-center">
            <button
              onClick={() => setIsOpen(!isOpen)} className="text-green-deep hover:text-green-soft focus:outline-none focus:ring-2 focus:ring-gold rounded-md p-2"
      ...
      {/* Mobile Menu Content */}
      <div id="mobile-menu" className={`md:hidden fixed top-[80px] left-0 w-full h-[calc(100vh-80px)] bg-ivory/98 backdrop-blur-xl overflow-y-auto transition-all duration-400 ease-[cubic-bezier(0.22,1,0.36,1)] ${
        ...
        <div className="flex flex-col px-10 py-10 space-y-8">
          {navLinks.map((link) => (
            <NavLink
              key={link.name}
              to={link.path}
              onClick={closeMenu} className={({ isActive }) =>
